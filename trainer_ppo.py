"""
PPO Trainer for A-Share Portfolio Management
============================================

滚动微调步进回测 + PPO 算法

训练流程：
1. 窗口切分：24月训练 + 1月验证 + 1月测试
2. PPO Rollout：Collect trajectory (T steps) -> GAE -> Update -> Clear
3. 增量学习：每窗口继承上一轮权重
4. 早停：基于验证集 Sharpe
"""
...
class PPOTrainer:
    ...
    def evaluate(self, env, agent):
        ...
        if not metrics:
            return {'sharpe': 0, 'total_return_pct': 0, 'max_drawdown': 0, 
                   'turnover_mean': 0, 'win_rate': 0, 'metrics_history': []}
    ...
    def train_window(self, train_df: pd.DataFrame, val_df: pd.DataFrame, 
        ...
        # 创建 Agent
        agent = PPOAgent(network, cfg_wrapper, self.config.device)
        ...
        for update in range(n_updates):
            ...
            with torch.no_grad():
                ...
                _, _, last_value = network(last_state_t, last_mask_t)
            ...
            if (update + 1) % max(1, n_updates // 10) == 0:
                ...
                print(f"  Update {update+1}/{n_updates}: "
                      ...
                      f"Entropy={loss_info['entropy_coef']:.4f}")
    ...
    def run(self):
        """执行滚动训练"""
        print(f"\n{'='*70}")
        print(f"🎯 PPO Rolling Walk-Forward Training")
        print(f"{'='*70}")
        print(f"Algorithm: PPO-Clip")
        print(f"Reward Scale: {self.trading.reward_scale}x (收益率放大)")
        print(f"Action Masking: Enabled (停牌/跌停屏蔽)")
